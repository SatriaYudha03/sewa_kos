...
class _KamarManagementScreenState extends State<KamarManagementScreen> {
  ...
  Future<void> _navigateToAddEditKamar({KamarKos? kamar}) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => AddEditKamarScreen(kosId: widget.kos.id, kamar: kamar)),
    );
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Apakah Anda yakin ingin menghapus kamar "$namaKamar"? Semua pemesanan terkait juga akan terhapus.'),
          actions: <Widget>[
            TextButton(
  ...
  Future<void> _deleteKamar(int kamarId) async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Menghapus kamar...'), duration: Duration(seconds: 1)),
    );
    try {
      ...
      if (mounted) {
        if (response['status'] == 'success') {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response['message'] ?? 'Kamar berhasil dihapus.'), backgroundColor: AppConstants.successColor),
          );
          _fetchKamarList(); // Refresh daftar kamar
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response['message'] ?? 'Gagal menghapus kamar.'), backgroundColor: AppConstants.errorColor),
          );
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text('Status: ${kamar.status}',
                          style: TextStyle(
                            color: kamar.status == 'tersedia' ? Colors.green : (kamar.status == 'terisi' ? Colors.orange : Colors.red),
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () => _navigateToAddEditKamar(kamar: kamar),
                          tooltip: 'Edit Kamar',
                        ),
                        IconButton(
                          icon: const Icon(Icons.delete, color: Colors.red),
                          onPressed: () => _confirmDeleteKamar(kamar.id, kamar.namaKamar),
                          tooltip: 'Hapus Kamar',
                        ),
                      ],
                    ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => _navigateToAddEditKamar(),
        backgroundColor: AppConstants.accentColor, // Warna berbeda dari FAB Kos utama
